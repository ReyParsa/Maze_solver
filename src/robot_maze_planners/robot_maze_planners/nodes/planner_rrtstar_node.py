...
from robot_maze_planners.planners.rrt_star_planner import RRTStarPlanner
from robot_maze_planners.utils.maze_helpers import parse_maze
from nav_msgs.msg import Odometry
import logging

class PlannerRRTStarNode(Node):
    def __init__(self):
        super().__init__('planner_rrtstar_node')
        # parameters (unified set)
        ...
        self.grid = None
        self.start = None
        self._last_logged_state = (None, None, None)
        ...
        self.get_logger().info(f"RRT* planner node started. goal={self.goal} step_size={self.step_size} max_iter={self.max_iter} radius={self.radius}")
    ...
    def try_plan(self):
        ...
        try:
            planner = RRTStarPlanner(grid, self.start, self.goal, self.step_size, self.max_iter, self.radius)
